...
$xml = simplexml_load_file('data/all-members-2010.xml') or die('cannot load file');
// $vals = $xml;
$i = 0;
foreach($xml AS $value) {
	$attr = $value->attributes();
	if (isset($import)) unset($import);
	$import['mys_id'] = $attr['id'];
	$import['house'] = $attr['house'];
	$import['title'] = $attr['title'];
	$import['firstname'] = $attr['firstname'];
	$import['lastname'] = $attr['lastname'];
	$import['constituency'] = $attr['constituency'];
	$import['party'] = $attr['party'];
	$import['from_date'] = $attr['from_date'];
	...
	$q = "SELECT * FROM `mp` WHERE `mys_id` = '".$attr['id']."'";
	if ($result = $mysqli->query($q)) {
		if ($result->num_rows == 0) {
			$i = "INSERT INTO `mp` (`" . implode('`,`',$cols) . "`) VALUES ('" . implode("','",$data) . "')";	
			if ($insert = $mysqli->query($i)) {
				echo $import['mys_id'] . " added\n";
			} else {
				echo $import['mys_id'] . " not added (" . $i . ")\n";
			}								
		} else {
			echo $import['mys_id'] . " exists\n";
		}
	}
	// print_r($attr);
	$i++;
